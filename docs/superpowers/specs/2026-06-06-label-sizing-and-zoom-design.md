# Design: Label sizing + visibility (zoom-responsive)
...
An upstream webmap (same shell lineage) solves this with a **screen-space,
zoom-responsive** font plus a **visibility/caching system** that scales to many
labels. The keppel and upstream coordinate magnitudes are comparable (both in the
~1000–2000 range), so the upstream formulas transfer to keppel's regime.

The zoom *mechanism* (fit + min/max clamps) is already essentially identical
between the two projects (keppel's `fitToBounds` is a refinement of the upstream
one). The "zoom feels off" symptom is dominated by unreadable labels, not the clamp;
`maxZoom` stays `2.5` and is tuned only if zoom-in still feels short after labels
are legible.

## Decision

**Port the upstream label handling in full** — font sizing *and* the
...
the host expects increasing label counts, and the upstream caching + idle-recompute
...
Add the upstream `#computeFontSize(scale, dpr)`:
...
- Starting values: `fontSize` and `minFontSize` seeded to read well in keppel's
  raw-coord space (upstream uses `8`/`8`); final values tuned during verification
...
- Remove the `fit` multiply in `#drawLabel` and the `fit` + `/scale` in
  ...
  directly (matching the upstream `#buildLabelRect`), so thinning rects finally match
  drawn labels.
...
Port these from upstream, **adapted to keppel's data model** (see "Data model
adaptation" below):
...
- `#computeLabelVisibility(...)` — recompute the visible/suppressed sets only when
  ...
  ([RectVisibility.js](../../../src/renderer/RectVisibility.js), shared with upstream).
...
**Faithful-port note:** in the upstream implementation the suppressed-label *fade*
path (`suppressedOpacity`) exists in `#renderLabel` but is latent — the render loop
...
The upstream project and keppel select/label nodes differently. **Keep keppel's
accessors**, port only the mechanism:

| Concern | Upstream | Keppel (keep) |
|---|---|---|
